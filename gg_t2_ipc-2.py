...
# Helper: derive identity from pid via /proc/<pid>/comm on Linux,
# or via `ps` on macOS. See shared.platform.proc_comm.
def identity_from_pid(pid: int | None) -> str:
    if pid is None:
        return "unknown"
    from shared.platform import proc_comm
    name = proc_comm(pid)
